...
(* ------------------------------------------------------------------ *)
(* Optional Wolfram/Arrays route                                       *)
(* ------------------------------------------------------------------ *)

(* TensorNetworks is the lower-level paclet and is typically touched first, so a
   container query routinely runs before Wolfram/Arrays is loaded.  Only the
   POSITIVE probe may be cached: a memoized negative would disable the route for
   the rest of the session even once the paclet is present, and arrayContainerQ
   would then keep declining genuine Arrays containers.  PacletFind is mocked
   empty to stand for "the paclet is not there yet"; the answer afterwards has
   to be the freshly probed one again, whether or not this kernel has it.

   PacletFind is Protected, and Internal`InheritedBlock preserves attributes, so
   the mock unprotects inside the block; the block restores both the attribute
   and the definitions on exit. *)

withoutArraysPaclet[body_] := Internal`InheritedBlock[{PacletFind},
    Unprotect[PacletFind];
    PacletFind[___] := {};
    body
]

SetAttributes[withoutArraysPaclet, HoldFirst]

VerificationTest[
    Block[{Wolfram`TensorNetworks`PackageScope`$arraysAvailable = False},
        With[{fresh = Wolfram`TensorNetworks`PackageScope`arraysAvailableQ[]},
            Block[{Wolfram`TensorNetworks`PackageScope`$arraysAvailable = False},
                {
                    withoutArraysPaclet[Wolfram`TensorNetworks`PackageScope`arraysAvailableQ[]],
                    Wolfram`TensorNetworks`PackageScope`$arraysAvailable,
                    Wolfram`TensorNetworks`PackageScope`arraysAvailableQ[] === fresh
                }
            ]
        ]
    ],
    {False, False, True},
    TestID -> "ArraysRoute_NegativeProbeIsNotMemoized"
]

(* The narrow local fallback still classifies the explicit heads this paclet
   handles on its own, so a query made while the paclet is missing is answered
   rather than declined. *)
VerificationTest[
    Block[{Wolfram`TensorNetworks`PackageScope`$arraysAvailable = False},
        withoutArraysPaclet[
            Map[
                Wolfram`TensorNetworks`PackageScope`arrayContainerQ,
                {{{1., 2.}, {3., 4.}}, SparseArray[{{1., 0.}, {0., 2.}}], NumericArray[{{1., 2.}, {3., 4.}}], "junk"}
            ]
        ]
    ],
    {True, True, True, False},
    TestID -> "ArraysRoute_LocalFallbackClassifiesExplicitHeads"
]
