...
2. **Path-cost convention.** *Audited 2026-05-08; resolved.*
   ...
   - **paclet Mathematica wrapper** (`TensorNetworks.wl:112-118`): default `Method -> "size"`, which explicitly overrides the Rust default. Per-step convention itself matches cotengra's `dtype=None` exactly.
...
3. **CI integration.** *Decided: separate step.* `run_external_validation.wl` runs as its own CI job (not part of `Tests/run_tests.wl` discovery) because it depends on local clones of the external packages and is slower than the in-paclet test suite. Local runs: `wolframscript -file Tests/external_validation/run_external_validation.wl`. Exit code 0 on all-pass, 1 on any failure or unexpected error.
