---
Template: TechNote
Name: SymmetrySubcontextTutorial
Title: A Working Tour of the Symmetry Functions
Context: Wolfram`TensorNetworks`Symmetry`
Paclet: Wolfram/TensorNetworks
URI: Wolfram/TensorNetworks/tutorial/SymmetrySubcontextTutorial
Keywords: [Young tableau, symmetry, irreducible representation, Schur-Weyl, tensor network, Riemann]
...
A short census of rank-2 tensors the reader has seen and the shape each one lives in follows. Each entry uses the same four-step pattern: build the tensor, test the right-shape projection against the original with `==` (returns `True`, confirming the tensor lives there), evaluate the wrong-shape projection alone (returns the zero matrix, confirming the orthogonal sector is empty), and read the symmetry class with the built-in detector. Anything mixed would split into a $\{2\}$-piece and a $\{1, 1\}$-piece that `YoungProject` decomposes; the census examples are all single-shape, so one of the two cells is trivially zero by construction.
...
Project $g$ onto the symmetric shape and compare with the original.

```wl
YoungProject[g, YoungTableau[{2}]] == g
```

Project $g$ onto the antisymmetric shape; the projection is identically zero.

```wl
YoungProject[g, YoungTableau[{1, 1}]]
```
...
Project $T$ onto the symmetric shape and compare with the original.

```wl
YoungProject[TSE, YoungTableau[{2}]] == TSE
```

Project $T$ onto the antisymmetric shape; the projection is identically zero.

```wl
YoungProject[TSE, YoungTableau[{1, 1}]]
```
...
```wl
YoungProject[covMat, YoungTableau[{2}]] == covMat
```

Project onto the antisymmetric shape; the projection is identically zero.

```wl
YoungProject[covMat, YoungTableau[{1, 1}]]
```
...
```wl
YoungProject[eps2, YoungTableau[{2}]]
```

Project onto the antisymmetric shape and compare with the original.

```wl
YoungProject[eps2, YoungTableau[{1, 1}]] == eps2
```
...
```wl
applySwap[a_, b_] := Block[{perm = Range[3]},
    perm[[{a, b}]] = perm[[{b, a}]];
    ArrayReshape[
        Transpose[
            Normal @ SparseArray[
...
```wl
applySwapND[a_, b_, n_, d_] := Block[{perm = Range[n]},
    perm[[{a, b}]] = perm[[{b, a}]];
    ArrayReshape[
        Transpose[
