...
// return value goes back the same way. Boundary conventions this implies:
//
//   - Index vectors read a NumericArray/PackedArray/ByteArray, not a plain
//     List, and WXF arrays are rectangular — so the jagged `inputs` term list
//     arrives flattened as (indices, per-term lengths). They cross the
//     boundary as Vec<i64>: that target accepts every lossless integer
//     source, including the ByteArray the WL side sends for an empty vector
//     (NumericArray cannot express one), and is narrowed to u32 here.
//   - Option<T> uses wolfram-serialize's enum encoding: "None" / {"Some", v}.
//   - f32 cannot read the Real64s BinarySerialize emits, so sizes and
//     temperatures cross the boundary as f64 and are narrowed here.
...
/// Narrow a wire-typed index vector to the internal u32 labels.
fn to_indices(v: Vec<i64>) -> Vec<u32> {
    v.into_iter().map(|i| i as u32).collect()
}

/// Rebuild the jagged term list from its flattened wire form.
fn unflatten(flat: Vec<i64>, lens: Vec<i64>) -> Vec<Vec<u32>> {
    let mut it = flat.into_iter();
    lens.into_iter()
        .map(|n| it.by_ref().take(n as usize).map(|i| i as u32).collect())
        .collect()
}

/// Narrow the wire-typed size dictionary to the internal `Dict<u32, f32>`.
fn to_size_dict(sizes: HashMap<i64, f64>) -> Dict<u32, f32> {
    sizes
        .into_iter()
        .map(|(k, v)| (k as u32, v as f32))
        .collect()
}

/// A contraction path as nested WL lists.
fn path_expr(path: SSAPath) -> Expr {
    let list = Symbol::new("System`List");
    let terms = path
        .into_iter()
        .map(|con| Expr::normal(list.clone(), con.into_iter().map(Expr::from).collect()))
        .collect();
    Expr::normal(list, terms)
}
...
fn find_subgraphs(
    ...
    let inputs = unflatten(inputs, input_lens);
    let output = to_indices(output);
    let size_dict = to_size_dict(sizes);
...
fn optimize_simplify(
    ...
    path_expr(if use_ssa.unwrap_or(false) {
        ssa_path
    } else {
...
fn optimize_greedy(
    ...
    let output = to_indices(output);
    let size_dict = to_size_dict(sizes);
    let costmod = costmod.map(|x| x as f32);
    let temperature = temperature.map(|x| x as f32);
    let max_neighbors = max_neighbors.map(|x| x as usize);
    let seed = seed.map(|x| x as u64);
    let n = inputs.len();
    let num_indices = size_dict.len();
    let max_nodes = 2 * n;
...
fn optimize_optimal(
    ...
    let output = to_indices(output);
    let size_dict = to_size_dict(sizes);
    let cost_cap = cost_cap.map(|x| x as Score);
    let n = inputs.len();
    let num_indices = size_dict.len();
    let max_nodes = 2 * n;
    let simplify = simplify.unwrap_or(true);
